...
def player_decision_1(hand, dealer_upcard):
    ...
    if total == 21 and len(hand) == 2:
        ...
        if total in range(18,22):
            action = "stand"
        elif dealer_upcard_value in range(9,11) and total == 18:
            action = "hit"
        elif dealer_upcard_value == 1 and total == 18:
            action = "hit"
        else:
            action = "hit"
    elif check_for_ace(hand) == False:
...
plt.title("Histogram of Cumulative Sums of 100 Games (AddStrat2)")
plt.xlabel("Cumulative Sum of 100 Games")
plt.savefig('addstrat2hist')
columns = [str(x) for x in range(100)]
plot = data_for_plot.plot(y=columns, use_index=True, legend=None)
plt.plot
plt.title("Cumulative Sums of 100 Rounds of 100 Games (AddStrat2)")
plt.xlabel("Game Number")
plt.ylabel("Cumulative Sum")
plt.savefig('addstrat2')
