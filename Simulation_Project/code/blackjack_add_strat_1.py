...
def player_decision_1(hand, dealer_upcard):
    ...
    if total == 21 and len(hand) == 2:
        ...
        if total in range(17,22):
            action = "stand"
        elif total == 12 and dealer_upcard_value in range(2,4):
            action = "hit"
        else:
            action = "hit"
    elif total >= 20:
...
plt.title("Histogram of Cumulative Sums of 100 Games (AddStrat1)")
plt.xlabel("Cumulative Sum of 100 Games")
plt.savefig('addstrat1hist')
columns = [str(x) for x in range(100)]
plot = data_for_plot.plot(y=columns, use_index=True, legend=None)
plt.plot
plt.title("Cumulative Sums of 100 Rounds of 100 Games (AddStrat1)")
plt.xlabel("Game Number")
plt.ylabel("Cumulative Sum")
plt.savefig('addstrat1')
